...
public class Euler011 {
	...
	private static long calc1(int max) {
		...
		for (int x = 0; x < SIZE; x++) {
			for (int y = 0; y < SIZE; y++) {
				for (Dir dir : Dir.DIRS) {
					if (Dir.checkLimit(dir, x, y, max, SIZE, SIZE)) {
						long prod = 1;
						for (int i = 0; i < max; i++) {
							int val = M[y + dir.dy * i][x + dir.dx * i];
							if (val == 0) {
								prod = 0;
								break;
							}
						...
						if (prod > best) {
							// if (prod == 70600674) {	System.out.println(x + ", " + y + ", dir:" + dir); }
							best = prod;
						}
					}
				}
	...
	static enum Dir {
		...
		public static final EnumSet<Dir> DIRS = EnumSet.of(Dir.N, Dir.NE, Dir.E, Dir.SE);
		...
		public static boolean checkLimit(Dir dir, int x0, int y0, int len, int xmax, int ymax) {

			int x = x0 + dir.dx * (len - 1);
			int y = y0 + dir.dy * (len - 1);
